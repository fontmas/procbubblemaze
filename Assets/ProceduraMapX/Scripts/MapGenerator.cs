...
namespace Utils.Map {
    ...
    public class MapGenerator : MonoBehaviour {
        ...
        public void CleanMap() {
            Transform[] objs = gameObject.GetComponentsInChildren<Transform>() as Transform[];
        ...
        public IEnumerator GenerateMap() {
            ...
            map = new int[mapSizeWH, mapSizeWH];
        ...
        void GenerateBorder() {
            int[,] borderedMap = new int[mapSizeWH + borderSize * 2, mapSizeWH + borderSize * 2];
            for (int x = 0; x < borderedMap.GetLength(0); x++) {
                for (int y = 0; y < borderedMap.GetLength(1); y++) {
                    if (x >= borderSize && x < mapSizeWH + borderSize && y >= borderSize && y < mapSizeWH + borderSize) {
                    ...
                    for (int count = 1; count <= amount; count++) {
                        ...
                        if (listCoords != null) {
                            ...
                            if (c != null) {
                                ...
                                float depth = map[c.tileX, c.tileY] == 0 ? transform.position.z + wallHeight - (transform.localScale.y * 0.75f) : transform.position.z - (transform.localScale.y * 0.75f);
                                map [c.tileX, c.tileY] = obj.ID;
                                tempObject.transform.position = CoordToWorldPoint(c.tileX, c.tileY, depth, mapOrientation);
                            ...
                            // There is no place the object so give up!
                        }
                    }
                }
            }
        ...
        public List<Coord> FindPlaceObjectInsideRoom(ref List<LineIDsToPlace> linesIds) {
            ...
            for (int y = 0; y < mapSizeWH; y++) {
                for (int x = 0; x < mapSizeWH; x++) {
                    bool testValidPosition = true;
                    for (int yID = 0; yID < linesIds.Count; yID++) {
                        for (int xID = 0; xID < linesIds[yID].line.Count; xID++) {
                            testValidPosition = testValidPosition && (map[x, y] == linesIds[yID].line[xID]);
                        }
                    }
                    if (testValidPosition == true) {
                        listPossiblePoints.Add(new Coord(x+(linesIds.Count-1)/2, y+(linesIds[0].line.Count - 1) / 2));
                    }
                }
            }
        ...
        private Vector3 CoordToWorldPoint(int x, int y, float depth = 0.0f, MapAxisOrientation axis = MapAxisOrientation.XwYhZd) {
            switch (axis) {
                case MapAxisOrientation.XwYhZd:
                    return new Vector3(((-mapSizeWH / 2.0f) + (-tileScale/2.0f) + x)*tileScale, ((-mapSizeWH / 2.0f) + (-tileScale/2.0f) + y)*tileScale, depth);
                case MapAxisOrientation.XwYdZh:
                    return new Vector3(((-mapSizeWH / 2) + .5f + x) * tileScale, depth, ((-mapSizeWH / 2) + .5f + y) * tileScale);
                case MapAxisOrientation.XdYwZh:
                    return new Vector3(depth, ((-mapSizeWH / 2) + .5f + x) * tileScale, ((-mapSizeWH / 2) + .5f + y) * tileScale);
                case MapAxisOrientation.XhYwZd:
                    return new Vector3(((-mapSizeWH / 2) + .5f + y) * tileScale, ((-mapSizeWH / 2) + .5f + x) * tileScale, depth);
                case MapAxisOrientation.XhYdZw:
                    return new Vector3(((-mapSizeWH / 2) + .5f + y) * tileScale, depth, ((-mapSizeWH / 2) + .5f + x) * tileScale);
                case MapAxisOrientation.XdYhZw:
                default:
                    return new Vector3(depth, ((-mapSizeWH / 2) + .5f + y) * tileScale, ((-mapSizeWH / 2) + .5f + x) * tileScale);
            }
        }

        List<List<Coord>> GetRegions(int tileType) {
            List<List<Coord>> regions = new List<List<Coord>>();
            int[,] mapFlags = new int[mapSizeWH, mapSizeWH];
        ...
        bool IsInMapRange(int x, int y) {
            return x >= 0 && x < mapSizeWH && y >= 0 && y < mapSizeWH;
        }
        ...
        void RandomFillMap() {
            ...
            for (int x = 0; x < mapSizeWH; x++) {
                for (int y = 0; y < mapSizeWH; y++) {
                    if (x == 0 || x == mapSizeWH - 1 || y == 0 || y == mapSizeWH - 1) {
                        map[x, y] = 1;
                    } else {
